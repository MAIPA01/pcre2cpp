...
namespace pcre2cpp {
	enum class regex_match_options_bits : uint32_t {
		...
		NO_UTF_CHECK =
		  PCRE2_NO_UTF_CHECK, // Do not check the subject for UTF validity(only relevant if PCRE2_UTF was set at compile time)
	...
	static _PCRE2CPP_CONSTEXPR17 regex_match_options operator|(const regex_match_options_bits opt0,
	  const regex_match_options_bits opt1) noexcept {
		return static_cast<regex_match_options>(opt0) | static_cast<regex_match_options>(opt1);
	}

	static _PCRE2CPP_CONSTEXPR17 regex_match_options operator|(const regex_match_options opts,
	  const regex_match_options_bits opt) noexcept {
		return opts | static_cast<regex_match_options>(opt);
	}
} // namespace pcre2cpp
	#endif
#endif